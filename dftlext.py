...
class Dftl(ftlbuilder.FtlBuilder):
    ...
    def sec_read(self, sector, count):
        ...
        self.read_time.append(time.time()-tic)
        return data
    ...
    def sec_write(self, sector, count, data = None):
        ...
        self.flash.write_pages(ppns = ppns_to_write, ppn_data = ppn_data,
                tag = DATA_USER)
        self.write_time.append(time.time()-tic)
        tic = time.time()
        self.garbage_collector.try_gc()
        self.gc_time.append(time.time()-tic)
    ...
    def sec_discard(self, sector, count):
        ...
        self.discard_time.append(time.time()-tic)
    ...
    def post_processing(self):
        ...
        prGreen("[INFO] Final avg read time: {} ms".format(cal_mean(self.read_time)*1000))
        prGreen("[INFO] Final avg write time: {} ms".format(cal_mean(self.write_time)*1000))
        prGreen("[INFO] Final avg discard time: {} ms".format(cal_mean(self.discard_time)*1000))
        prGreen("[INFO] Final avg GC time: {} ms".format(cal_mean(self.gc_time)*1000))
